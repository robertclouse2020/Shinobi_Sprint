...
public class PlayerMove : MonoBehaviour
{
    ...
    //movement
    private float accel = 150f;                   //acceleration/deceleration in air or on the ground
    private float airAccel = 18f;
    private float decel = 5f;
    private float airDecel = 0.5f;

    // movement manipulator mulitpliers
    public float PlayerSpeed = 0.5f;         // player speed. //used to speed up or slow down the player. //should be keeped below 1.5 for quick/sprint and higher for walking/crawling
    public float JumpMultiplier = 1.1f;     // keep this value below 2 or you will go to space. 1.1 is defualt
    ...
    //jumping
    public Vector3 jumpForce = new Vector3(0, 25, 0);       //normal jump force
    ...
    void Update()
    {
        ...
        if (PlayerSpeed <= 1.5)
        {
            curAccel /= PlayerSpeed;                // keeps the player moving at a caterpillers pace
        }
        else
        {
            PlayerSpeed = 1.5f;
